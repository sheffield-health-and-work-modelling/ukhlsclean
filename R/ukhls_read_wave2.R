...
ukhls_read_wave2 <- function(
  ...
  id_vars          <- Hmisc::Cs(pidp, pid, b_hidp, b_pno, b_psu, b_strata, b_istrtdaty, b_istrtdatm, b_istrtdatd, b_ivfio)
  ...
  data.income <- data.table::fread(
    paste0(path, "/b_income.tab"),
    showProgress = FALSE,
    na.strings = c("NA", "", "-1", "-2", "-6", "-7", "-8", "-9", "-90", "-90.0", "N/A")
  )
  colnames(data.income) <- sub("^b_", "", colnames(data.income))
