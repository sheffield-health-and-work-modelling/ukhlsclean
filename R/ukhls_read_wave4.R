...
ukhls_read_wave4 <- function(
  ...
  id_vars          <- Hmisc::Cs(pidp, pid, d_hidp, d_pno, d_psu, d_strata, d_istrtdaty, d_istrtdatm, d_istrtdatd, d_ivfio)
  ...
  work_vars        <- Hmisc::Cs(d_paygu_dv, d_payg_dv, d_jbhrs, d_seearngrs_dv, d_jbsic07_cc, d_jbot, d_jbotpd, d_jshrs,
...
>>>>>>> 9c47886c481433cab8298029c1f6bc6ece6000ef
  names <- tolower(names)
  ...
  data.income <- data.table::fread(
    paste0(path, "/d_income.tab"),
    showProgress = FALSE,
    na.strings = c("NA", "", "-1", "-2", "-6", "-7", "-8", "-9", "-90", "-90.0", "N/A")
  )
  colnames(data.income) <- sub("^d_", "", colnames(data.income))
