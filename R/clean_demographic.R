...
clean_demographic <- function(data = NULL) {
  ...
  data[ , age_12cat := c("16-19", "20-24", "25-29", "30-34", "35-39", "40-44",
                         ...
                         "70+")[findInterval(dvage, c(16, seq(20,65,5), 70))]]
  data[ , age_12cat := as.factor(age_12cat)]

  data[ , age_5cat := c("16-24", "25-34", "35-49", "50-64", "65+")[findInterval(dvage, c(16, 25, 35, 50, 65))]]
  data[ , age_5cat := as.factor(age_5cat)]

  setnames(data, "dvage", "age")
  ...
  data[gor_dv == 1  , gor := "north_east"]
  data[gor_dv == 2  , gor := "north_west"]
  data[gor_dv == 3  , gor := "yorkshire"]
  data[gor_dv == 4  , gor := "east_midlands"]
  data[gor_dv == 5  , gor := "west_midlands"]
  data[gor_dv == 6  , gor := "east"]
  data[gor_dv == 7  , gor := "london"]
  data[gor_dv == 8  , gor := "south_east"]
  data[gor_dv == 9  , gor := "south_west"]
  data[gor_dv == 10 , gor := "wales"]
  data[gor_dv == 11 , gor := "scotland"]
  data[gor_dv == 12 , gor := "northern_ireland"]

  data[,gor := as.factor(gor)]

  data[gor_dv %in% 1:9, country := "england"]
  data[gor_dv == 10 , country := "wales"]
  data[gor_dv == 11 , country := "scotland"]
  data[gor_dv == 12 , country := "northern_ireland"]
  ...
  data[urban_dv == 1  , area := "urban"]
  data[urban_dv == 2  , area := "rural"]
  ...
  data[hiqual_dv == 1, hiqual := "degree"]
  data[hiqual_dv == 2, hiqual := "other_he"]
  data[hiqual_dv == 3, hiqual := "alevel"]
  data[hiqual_dv == 4, hiqual := "gcse"]
  data[hiqual_dv == 5, hiqual := "other_qual"]
  data[hiqual_dv == 9, hiqual := "no_qual"]
